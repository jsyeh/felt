...
float[][][] StrandTrans = new float[12][256*256][segmentN+1]; //strand的vertex有31個
void setup(){
  size(1024, 1024);
  ...
  for(int ix=0; ix<NX; ix++){
    for(int iy=0; iy<NY; iy++){ //決定平面的點
      ...
      for(int iz=0; iz<NZ; iz++){ //往z方向打穿，也是從 imin一直打到i為止
        ...
        prevTrans *= exp(-density[ix][iy][iz]/1000*f*ds);
      }
    }
  }
...
float Interpolate(float x, float y, float z){
  int i = int(y/(NY/256)), j = int(x/(NX/256));
  if(i<0 || j<0 || i>=256-1 || j>=256-1) return 0;
...
/*  for(int i=0; i<NX-1; i++){
    for(int j=0; j<NY-1; j++){
      for(int k=0; k<NZ-1; k++){ //這裡的 i,j,k只是要找到x,y,z對應的虛擬中心格子
